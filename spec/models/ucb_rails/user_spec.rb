...
describe UcbRails::User do
  ...
  it '#superuser!' do
    user = UcbRails::User.create!(ldap_uid: 1)
  ...
  it '.active' do
    active = UcbRails::User.create(ldap_uid: 1)
    inactive = UcbRails::User.create(ldap_uid: 2, inactive_flag: true)
    expect(UcbRails::User.active).to eq([active])
  end

  it '.superuser' do
    superuser = UcbRails::User.create(ldap_uid: 1, superuser_flag: true)
    not_superuser = UcbRails::User.create(ldap_uid: 2)
    expect(UcbRails::User.superuser).to eq([superuser])
  end
end
