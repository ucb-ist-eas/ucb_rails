...
describe UcbRailsControllerMethods do
  ...
  describe 'setting Thread.current[:current_user]' do
    it "set in log request, cleared in remove_user_settings" do
      ...
      expect(UcbRails::UserSessionManager::Base.current_user).to eq(user_mock)
      controller.remove_user_settings
      expect(UcbRails::UserSessionManager::Base.current_user).to eq(nil)
    end
  end
